...
- Print the PDF: `output/Ag_Plot_Plan_17054_Handlebar_rev21.pdf` (always the
...
The crop areas are drawn clipped to the parcel boundary on the sheet, so no red
area crosses a property line; the tabulated square footages are your numbers.
...
- **Greenhouse** is labelled **AS-BUILT** — the county's own term from PDS 090
  for a structure built without a permit, so it discloses the status without
  flagging it twice. Ask whether it qualifies for the agricultural building
  exemption.
...
Check `output/Verification_Overlay_v4.png` after any geometry change — it draws
